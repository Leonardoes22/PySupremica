...
for i in range(len(heightMap)):
    for j in range(len(heightMap[i])):
        ...
        ga.newEvent("a"+str(i+1)+"."+str(j+1) + "_r2", kind="CONTROLLABLE")

        # Set height variables
        mod.addVariable("h"+str(i+1)+"."+str(j+1),"h"+str(i+1)+"."+str(j+1)+ "==0", [0,"H"+str(i+1)+"."+str(j+1)])
...
gr.setEdgeGuard("IN", "OUT", "x=="+str(inTiles[0][0])+"& y=="+ str(inTiles[0][1]))
...
g_b.addEdge("S1","S0", ga()["a1.1"])
g_b.addEventToEdge("S1","S0", ga()["a1.2"])
...
for i in range(1,largura+1):
    ...
    for j in range(1,comprimento+1):
        ...
        for n in neighborhood((i,j)):
            if(inGrid(n)):
                guard += "(x=="+str(n[0])+" & y=="+str(n[1])+") & (h"+coordEnc(i,j)+"==h"+coordEnc(n[0],n[1])+")|"
...
for i in range(1,largura+1):
    for j in range(1,comprimento+1):
        ...
        if(j>1):
            l_guard+= "(x=="+str(i)+" & y=="+str(j)+") & ((-1 <= h"+coordEnc(i,j)+"-h"+coordEnc(i,j-1)+")&(h"+coordEnc(i,j)+"-h"+coordEnc(i,j-1)+" <= 1))|"
        if(i>1):
            u_guard+= "(x=="+str(i)+" & y=="+str(j)+") & ((-1 <= h"+coordEnc(i,j)+"-h"+coordEnc(i-1,j)+")&(h"+coordEnc(i,j)+"-h"+coordEnc(i-1,j)+" <= 1))|"
        if(j<comprimento):
            r_guard+= "(x=="+str(i)+" & y=="+str(j)+") & ((-1 <= h"+coordEnc(i,j)+"-h"+coordEnc(i,j+1)+")&(h"+coordEnc(i,j)+"-h"+coordEnc(i,j+1)+" <= 1))|"
        if(i<largura):
            d_guard+= "(x=="+str(i)+" & y=="+str(j)+") & ((-1 <= h"+coordEnc(i,j)+"-h"+coordEnc(i+1,j)+")&(h"+coordEnc(i,j)+"-h"+coordEnc(i+1,j)+" <= 1))|"
